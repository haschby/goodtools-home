...
export function PaginationAction<T extends { id: string }>() {

    const { pagination, fetchData, activeStatus } = useDataTable<T>();
        ...
        <section className="rounded-b-xl bg-gray-50 p-3 flex flex-row gap-2 items-center justify-between border border-gray-200">
            <div className="flex flex-row gap-2 items-center">
                ...
                { options.map(
                    ...
                    const isDisabled =
                        pagination?.total_by_status
                        && pagination?.total_by_status < parseInt(option.value);
